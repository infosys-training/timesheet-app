...
// Run a work entry listing query and return the rows
function listEntries(db, res, query, params) {
...
// Look up an entry, optionally scoped to its owner, and hand it to onFound
function loadEntry(db, res, workEntryId, ownerEmail, onFound) {
  const scoped = Boolean(ownerEmail);
  const query = scoped
    ? 'SELECT id, status FROM work_entries WHERE id = ? AND user_email = ?'
    : 'SELECT id, status FROM work_entries WHERE id = ?';

  db.get(query, scoped ? [workEntryId, ownerEmail] : [workEntryId], (err, row) => {
    if (err) {
      console.error('Database error:', err);
      return res.status(500).json({ error: 'Internal server error' });
    }
...
router.get('/', (req, res) => {
  ...
  listEntries(db, res, query, params);
});
...
router.put('/:id', (req, res, next) => {
  try {
    ...
    loadEntry(db, res, workEntryId, req.userEmail, (row) => {
...
function reviewEntry(targetStatus, successMessage) {
  return (req, res) => {
    ...
    loadEntry(db, res, workEntryId, null, (row) => {
