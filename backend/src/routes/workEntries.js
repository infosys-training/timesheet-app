...
function sendWorkEntry(db, id, res, message, responseStatus = 200) {
  db.get(
    ...
    (err, row) => {
      if (err) {
        ...
        const retrievalMessage = `${message.replace(' successfully', '')} but failed to retrieve`;
        return res.status(500).json({ error: retrievalMessage });
      }
...
function handleTransition(action, req, res) {
  ...
  db.get(lookupQuery, lookupParams, (err, row) => {
    ...
    db.run(query, params, updateErr => {
      ...
      sendWorkEntry(db, workEntryId, res, message);
    });
  });
}
      ...
      (err, row) => {
        ...
        db.run(
          ...
          function(insertErr) {
            ...
            sendWorkEntry(db, this.lastID, res, 'Work entry created successfully', 201);
          }
        );
      }
    );
        ...
        const performUpdate = () => {
          ...
          db.run(
            ...
            updateErr => {
              ...
              sendWorkEntry(db, workEntryId, res, 'Work entry updated successfully');
            }
          );
        };
