...
router.get('/client/:clientId', (req, res) => {
  ...
  db.get(
    ...
    (err, client) => {
      ...
      db.all(
        `SELECT id, hours, description, date, status, submitted_at,
...
router.get('/export/csv/:clientId', (req, res) => {
  ...
  db.get(
    ...
    (err, client) => {
      ...
      // Get work entries
      db.all(
        `SELECT hours, description, date, status, submitted_at,
                reviewed_at, reviewed_by, rejection_reason, created_at
         FROM work_entries 
         WHERE client_id = ? AND user_email = ? 
         ORDER BY date DESC`,
        [clientId, req.userEmail],
