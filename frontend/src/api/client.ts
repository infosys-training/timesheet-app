...
class ApiClient {
  ...
  async getWorkEntries(clientId?: number, status?: WorkEntryStatus) {
    const params: { clientId?: number; status?: WorkEntryStatus } = {};
    if (clientId) {
  ...
  async approveWorkEntry(id: number, note?: string) {
    const response = await this.client.post(`/api/work-entries/${id}/approve`, { note });
    return response.data;
  }

  async rejectWorkEntry(id: number, note?: string) {
    const response = await this.client.post(`/api/work-entries/${id}/reject`, { note });
    return response.data;
  }
